...
class BenchmarkRunner:
    ...
    def run_read_workloads(self) -> Dict[str, Any]:
        ...
        for _ in range(self.warmup_iterations):
            ...
            self.adapter.run_query(QueryWorkloads.get_1hop_traversal(start_id))
            self.adapter.run_query(QueryWorkloads.get_point_lookup(start_id))
        ...
        for _ in range(self.iterations):
            start_id = random.randint(1, self.num_nodes)
            q = QueryWorkloads.get_1hop_traversal(start_id)
            _, lat = self.adapter.run_query(q)
            l_1hop.append(lat)
        ...
        for _ in range(self.iterations):
            start_id = random.randint(1, self.num_nodes)
            q = QueryWorkloads.get_2hop_traversal(start_id)
            _, lat = self.adapter.run_query(q)
            l_2hop.append(lat)
        ...
        for _ in range(self.iterations):
            start_id = random.randint(1, self.num_nodes)
            q = QueryWorkloads.get_3hop_traversal(start_id)
            _, lat = self.adapter.run_query(q)
            l_3hop.append(lat)
        ...
        for _ in range(self.iterations):
            ...
            q = QueryWorkloads.get_point_lookup(user_id)
            _, lat = self.adapter.run_query(q)
            l_lookup.append(lat)
        ...
        for _ in range(self.iterations):
            ...
            q = QueryWorkloads.get_indexed_lookup(cat)
            _, lat = self.adapter.run_query(q)
        ...
        l_agg = []
        for _ in range(self.iterations):
            q = QueryWorkloads.get_aggregation_groupby()
            _, lat = self.adapter.run_query(q)
            l_agg.append(lat)
            ...
            def worker_client():
                ...
                while time.time() < stop_time:
                    ...
                    if random.random() < 0.8:
                        ...
                        q = QueryWorkloads.get_write_transaction(src, dst, round(random.uniform(0.1, 1.0), 2))
