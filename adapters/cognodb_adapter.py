...
Uses official Neo4j Bolt driver with TLS (bolt+s://) to communicate with CognoDB Cloud instance.
User: cognodb
"""
...
class CognoDBAdapter(BaseGraphAdapter):
    def __init__(self, uri: str, user: str = "cognodb", password: str = "", advertised_specs: str = "Burstable 0.5 vCPU, 256 MB RAM"):
        super().__init__(name="CognoDB Cloud", advertised_specs=advertised_specs)
        self.uri = uri
        self.user = user
    ...
    def create_schema_and_indexes(self) -> bool:
        ...
        try:
            with self.driver.session() as session:
                # Index on User id and User category
            ...
            try:
                ...
                print(f"[{self.name}] Note during index creation: {e2}")
                return False
    ...
    def ingest_nodes_batch(self, nodes_batch: List[Tuple]) -> int:
        ...
        # Format: list of dicts for UNWIND batching
        data = [
            {
                "id": row[0],
                "username": row[1],
    ...
    def get_storage_footprint(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "stored_size_mb": "not observable (Cloud managed)",
                "memory_usage_mb": "256 MB (c0 Free Tier cap)",
                "details": info
            }
        except Exception:
            return {"stored_size_mb": "not observable", "memory_usage_mb": "256 MB (c0 Free Tier)"}
